...
# Natural Learning Processing Tools
# nltk lets you handle large amounts of text in optimized manner
...
class CyberbullyingDetectionEngine:
    ...
    def _simplify(self, corpus):
        """ Takes in a list of strings (corpus) and removes stopwords (words that comprise most of the English language)
            and it converts it to lowercase as CountVectorizer is case-sensitive. it also removes non-alphanumeric characters and it stems
            each word, i.e. running becomes run """
    ...
    def model_metrics(self, features, tags):
    ...
    def load_corpus(self, path, corpus_col, tag_col):
        """Takes in a path to pickled pandas dataframe, the name of corpus column, and the name of tag column,
...
if __name__ == '__main__':
    reddit = praw.Reddit(
        client_id = '6gmFOZwdcGj7mQ',
        client_secret = 'NPx32s_YDAELORoFj8YfNPPUuFk',
        user_agent = 'reddit_script by /u/navy_weirdo')
